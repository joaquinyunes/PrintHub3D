...
export default function ScrollAnimation({
  ...
  return (
    <div
      id={sectionId}
      ref={containerRef}
      style={{ height: "500vh", position: "relative", backgroundColor: "#0a0a0f" }}
    >
      <video
        ref={videoRef}
        src={videoSrc}
      ...
      <div style={{
        ...
        overflow: "hidden", backgroundColor: "#0a0a0f", zIndex: 2
      }}>
        {loading && (
          <div style={{
            position: "absolute", inset: 0, display: "flex",
            flexDirection: "column", alignItems: "center", justifyContent: "center",
            zIndex: 30, backgroundColor: "#0a0a0f"
          }}>
            <div style={{ fontSize: "56px", marginBottom: "1.5rem", filter: `drop-shadow(0 0 20px ${accentColor})` }}>🏆</div>
            <div style={{
            ...
            <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "13px" }}>Cargando experiencia...</p>
          </div>
        )}
        ...
        <div style={{
          ...
          transition: "all 0.7s cubic-bezier(0.25, 0.46, 0.45, 0.94)", zIndex: 10
        }}>
          <div className="max-w-3xl mx-auto text-center">
            <div style={{
              display: "inline-flex", alignItems: "center", gap: "0.5rem",
              background: `${accentColor}20`, border: `1px solid ${accentColor}50`,
              borderRadius: "9999px", padding: "0.6rem 1.5rem", marginBottom: "1.25rem"
            }}>
              <span style={{ fontSize: "18px" }}>🏆</span>
              <span style={{ color: accentColor, fontSize: "12px", fontWeight: "700", letterSpacing: "0.1em", textTransform: "uppercase" }}>{badge}</span>
            </div>
            <h2 style={{ fontSize: "clamp(3rem, 8vw, 5rem)", fontWeight: "900", color: "white", lineHeight: 1, marginBottom: "0.75rem", letterSpacing: "-0.03em" }}>{title}</h2>
            <p style={{ fontSize: "1.125rem", color: "rgba(255,255,255,0.55)", marginBottom: "2rem" }}>{subtitle}</p>
            <div style={{
              display: "inline-block", background: `${accentColor}15`,
              backdropFilter: "blur(10px)", borderRadius: "1.25rem",
              padding: "1rem 2.5rem", marginBottom: "1.5rem", border: `1px solid ${accentColor}40`
            }}>
              <span style={{ fontSize: "2.75rem", fontWeight: "900", color: accentColor }}>{price}</span>
            </div>
            <button
              ...
              style={{
                ...
                color: "white", padding: "1.1rem 2.5rem", borderRadius: "0.75rem",
                fontWeight: "700", fontSize: "15px", border: "none", cursor: "pointer",
                boxShadow: `0 8px 30px ${accentColor}40`
              }}
            >
              <span style={{ fontSize: "18px" }}>💬</span> Consultar ahora
            </button>
          </div>
        </div>
      </div>
