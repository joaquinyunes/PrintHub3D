...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
      try {
          ...
          if (resOrders.ok && resPrinters.ok) {
              ...
              const orders = Array.isArray(rawOrders) ? rawOrders : (rawOrders.items || []);
              ...
              const salesItems = Array.isArray(rawSales) ? rawSales : (rawSales.items || []);
              const settingsData = resSettings.ok ? await resSettings.json() : {};

              const activeOrders = orders.filter((o:any) => o.status === 'in_progress');

              const enrichedPrinters = printersRaw.map((p: any) => {
                  const order = activeOrders.find((o:any) => String(o.printerId || o.printer?._id || o.printer) === String(p._id));
  ...
  if (loading) return <div className="flex h-screen items-center justify-center bg-[#050505] text-white"><Activity className="animate-spin text-blue-500 h-10 w-10"/></div>;

  const goalPercent = Math.min((stats.revenue / monthlyGoal) * 100, 100);

  return (
    <div className="min-h-screen w-full bg-[#050505] text-white font-sans flex justify-center overflow-x-hidden">
      <div className="w-full max-w-7xl p-8 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-[#0a0a0a] border border-white/5 rounded-3xl p-6 shadow-2xl flex flex-col">
                ...
                <div className="grid grid-cols-2 gap-4 flex-1">
                    <ActionTile icon={Plus} label="Nuevo Pedido" color={{bg:'bg-blue-500/10', text:'text-blue-400'}} onClick={() => router.push('/admin/orders')} />
                    <ActionTile icon={Spool} label="Cargar Stock" color={{bg:'bg-purple-500/10', text:'text-purple-400'}} onClick={() => setIsFilamentModalOpen(true)} />
                    <ActionTile icon={Boxes} label="Inventario" color={{bg:'bg-orange-500/10', text:'text-orange-400'}} onClick={() => router.push('/admin/products')} />
                    <ActionTile icon={Bot} label="Config Web" color={{bg:'bg-emerald-500/10', text:'text-emerald-400'}} onClick={() => router.push('/admin/home')} />
                </div>
            </div>
        </div>
        ...
        <div className="bg-[#0f0f0f] border border-white/5 rounded-3xl p-8 shadow-xl">
            ...
            <div className="flex gap-2 mb-6">
                <input className="flex-1 bg-[#1a1a1a] border border-white/10 rounded-xl px-4 py-3 text-sm text-white outline-none focus:border-pink-500" placeholder="Escribe una tarea..." value={newTaskText} onChange={(e) => setNewTaskText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddTask()} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {tasks.length === 0 ? <p className="text-center text-gray-600 text-xs py-4 uppercase tracking-widest col-span-2">No hay tareas</p> : tasks.map(task => (
                    <div key={task._id} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${task.completed ? 'bg-black/20 border-white/5 opacity-50' : 'bg-[#141414] border-white/10 hover:border-pink-500/30'}`}>
                        <div className="flex items-center gap-4">
                            <button onClick={() => toggleTask(task._id, task.completed)}>{task.completed ? <CheckSquare size={20} className="text-pink-500"/> : <Square size={20} className="text-gray-500"/>}</button>
                            <span className={`text-sm ${task.completed ? 'line-through text-gray-500' : 'text-white'}`}>{task.text}</span>
                        </div>
                        <button onClick={() => deleteTask(task._id)} className="text-gray-600 hover:text-red-500 p-2"><Trash2 size={14}/></button>
                    </div>
                ))}
            </div>
        </div>
