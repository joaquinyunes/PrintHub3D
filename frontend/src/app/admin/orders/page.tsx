...
interface Order {
  ...
  status: string;
  createdAt: string;
  items: (OrderItem & { printedQuantity?: number })[];
...
const TABS = [
  ...
  },
  {
    ...
    description: 'Finalizados en espera',
    icon: PackageCheck,
...
export default function OrderListPage() {
  ...
  const [settingsCustomCodes, setSettingsCustomCodes] = useState<
      ...
      try {
        ...
        if (resS.ok) {
          try {
            ...
            const cc = settings?.homepageSections?.customCodes ?? settings?.customCodes ?? [];
        ...
        if (dateStr) {
          ...
          if (activeTab === 'history') key = `📅 ${orderDate.toLocaleDateString('es-AR')}`;
            ...
            if (diffDays < 0) key = '⚠️ VENCIDOS';
              ...
              key = `📅 ${orderDate.toLocaleDateString('es-AR', { weekday: 'long', day: 'numeric', month: 'long' })}`;
          }
        }
      }
  ...
  const handleSaveOrder = async () => {
    ...
    const payload = {
      ...
      items: cart,
      files,
      total: cart.reduce((acc, i) => acc + Number(i.price) * Number(i.quantity), 0),
    };
  ...
  const confirmDelivery = async () => {
    ...
    try {
      const res = await apiFetch(`/api/orders/${deliverOrder._id}/register-sale`, {
        method: 'POST',
  ...
  const changeStatus = async (id: string, status: string) => {
    ...
    try {
      ...
      setOrders((cur) => cur.map((o) => (o._id === id ? { ...o, ...updated } : o)));
    } catch (e) {
      console.error(e);
      setOrders(prev);
      alert('No se pudo cambiar el estado del pedido. Reintentá en unos segundos.');
    }
  };
  ...
  const handlePrintItem = async () => {
    ...
    try {
      const res = await apiFetch(`/api/orders/${printOrder._id}/print-item`, {
        method: 'POST',
      ...
      if (p && itemInput.qty > 0) {
        setCart([
          ...cart,
          {
            ...
            quantity: Number(itemInput.qty),
            isCustom: false,
            printTimeMinutes: Number(itemInput.printTimeMinutes) || 30,
          },
        ]);
        setItemInput({ ...itemInput, qty: 1, printTimeMinutes: 30 });
      }
    } else {
      if (itemInput.customName && itemInput.customPrice) {
        const productName = itemInput.productType
          ? `${itemInput.customName} (${itemInput.productType})`
          : itemInput.customName;
        setCart([
          ...cart,
          {
            productName,
            price: Number(itemInput.customPrice),
            quantity: Number(itemInput.qty) || 1,
            isCustom: true,
            printTimeMinutes: Number(itemInput.printTimeMinutes) || 30,
            productType: itemInput.productType || undefined,
          },
        ]);
        setItemInput({
          ...itemInput,
    ...
    <div className="min-h-screen bg-[#050505] p-8 text-white font-sans selection:bg-blue-500/30">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end md:items-center mb-10 gap-6">
          <div>
            <h1
              className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-purple-400 to-white tracking-tighter mb-2"
              style={{ textShadow: '0px 0px 30px rgba(59,130,246,0.3)' }}
            >
              PEDIDOS
            </h1>
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <div className="relative group flex-1 md:w-64">
              ...
              <input
                type="text"
                placeholder="BUSCAR..."
                className="w-full bg-white/5 border border-white/10 rounded-full py-4 pl-12 pr-6 text-xs font-bold text-white outline-none focus:bg-white/10 focus:border-blue-500/50 transition-all uppercase tracking-wider"
                value={searchTerm}
            ...
            <button
              onClick={openNew}
              className="bg-white text-black px-8 py-4 rounded-full font-black flex items-center gap-3 hover:bg-blue-50 hover:scale-105 transition-all shadow-[0_0_20px_rgba(255,255,255,0.3)] active:scale-95 text-xs uppercase tracking-widest whitespace-nowrap"
            >
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative pb-4 px-6 text-sm font-bold uppercase tracking-widest transition-all flex items-center gap-3 group min-w-fit ${isActive ? 'text-white' : 'text-gray-600 hover:text-gray-300'}`}
              >
                <tab.icon
                  className={`h-5 w-5 ${isActive ? tab.color : 'text-gray-600 group-hover:text-gray-400'}`}
                />
                <div className="flex flex-col items-start">
                  <span>{tab.label}</span>
                  <span className="text-[9px] normal-case opacity-50 font-medium tracking-normal">
                    {tab.description}
                  </span>
                </div>
                {count > 0 && (
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full ${isActive ? 'bg-white text-black' : 'bg-white/10 text-gray-500'}`}
                  >
                    {count}
                  </span>
                )}
                {isActive && (
                  <div
                    className={`absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-${tab.color.split('-')[1]}-500 to-transparent shadow-[0_-5px_10px_${tab.color.split('-')[1]}]`}
                  />
                )}
              </button>
            );
        ...
        <div className="space-y-16 pb-20">
          {Object.keys(groupedOrders).length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 opacity-30">
              <Package size={64} className="mb-4 text-gray-500" />
            ...
            sortedGroupKeys.map((dateLabel) => (
              ...
              >
                <div className="flex items-center gap-4 mb-6">
                  <div
                    className={`h-px flex-1 ${dateLabel.includes('VENCIDOS') ? 'bg-red-500/50' : 'bg-white/10'}`}
                  ></div>
                  <h3
                    className={`text-xl font-black uppercase tracking-tight flex items-center gap-3 ${dateLabel.includes('VENCIDOS') ? 'text-red-500 animate-pulse' : dateLabel.includes('HOY') ? 'text-yellow-400' : 'text-white'}`}
                  >
                    {dateLabel.includes('VENCIDOS') && <AlertTriangle className="h-6 w-6" />}
                    {dateLabel}
                  </h3>
                  <div
                    className={`h-px flex-1 ${dateLabel.includes('VENCIDOS') ? 'bg-red-500/50' : 'bg-white/10'}`}
        ...
        {isDeliverModalOpen && deliverOrder && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in">
            <div className="w-full max-w-md bg-[#0f0f0f] border border-white/10 rounded-3xl p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
              <h2 className="text-xl font-bold text-white mb-2 uppercase tracking-wide flex items-center gap-2">
                <CheckCircle2 className="text-emerald-500" /> Confirmar Entrega
              </h2>
              <p className="text-xs text-gray-500 mb-6 leading-relaxed">
                El pedido se marcará como entregado y se guardará en ventas.
              </p>
              <div className="bg-[#141414] p-5 rounded-xl border border-white/5 mb-6 space-y-4">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400 font-medium">Total Venta</span>
                  <span className="font-bold text-white">
                    ${(deliverOrder.total ?? 0).toLocaleString()}
                  </span>
                </div>
                <div>
                  <label className="text-[10px] font-bold text-blue-400 uppercase tracking-wider block mb-2">
                    Costo Real (Luz + Material)
                  </label>
                  <input
                    autoFocus
                    type="number"
                    className="w-full bg-black/50 border border-white/10 rounded-lg py-3 pl-4 text-white outline-none focus:border-blue-500 transition-all font-bold"
                    placeholder="0"
                    value={finalCost}
                ...
                <div className="border-t border-white/5 pt-3 flex justify-between text-sm">
                  ...
                  <span
                    className={`font-bold ${(deliverOrder.total ?? 0) - Number(finalCost) > 0 ? 'text-emerald-400' : 'text-red-400'}`}
                  >
                    ${((deliverOrder.total ?? 0) - Number(finalCost)).toLocaleString()}
                  </span>
                </div>
              </div>
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-bold text-gray-400 uppercase tracking-wider"
                >
                  Cancelar
                </button>
                <button
                  onClick={confirmDelivery}
                  className="flex-1 py-3 rounded-xl bg-green-600 hover:bg-green-500 text-white text-xs font-bold uppercase tracking-wider shadow-lg shadow-green-900/20"
                >
                  Confirmar
                </button>
              </div>
            ...
            <div className="w-full max-w-6xl bg-[#0f0f0f] border border-white/10 rounded-3xl shadow-2xl flex flex-col max-h-[90vh]">
              <div className="p-8 border-b border-white/5 flex justify-between items-center bg-[#141414]">
                <h2 className="text-2xl font-black text-white flex items-center gap-3 uppercase tracking-tight">
                  {editingOrderId ? (
                    <Pencil className="text-yellow-500 h-6 w-6" />
                  ) : (
                ...
                <button onClick={() => setIsModalOpen(false)}>
                  <X className="h-6 w-6 text-gray-500 hover:text-white transition-colors" />
                </button>
              </div>
              <div className="p-8 overflow-y-auto flex-1 grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <div className="lg:col-span-5 space-y-8">
                  <div className="bg-[#141414] p-6 rounded-2xl border border-white/5 space-y-6">
                    <div>
                      <label className="text-[10px] font-bold text-gray-500 uppercase mb-2 block tracking-widest">
                        Cliente
                      </label>
                      <input
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-gray-500 uppercase mb-2 block tracking-widest flex items-center gap-2">
                        <MessageSquare size={12} /> Link del Chat
                      </label>
                      <input
                        className="w-full bg-black/50 border border-white/10 rounded-xl p-3 text-xs text-gray-300 outline-none focus:border-blue-500"
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-purple-400 uppercase tracking-widest mb-2 block">
                      Fecha de Entrega
                    </label>
                    <input
                      type="date"
                      className="w-full bg-black/50 border border-white/10 rounded-xl p-4 text-white outline-none focus:border-purple-500"
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <input
                        placeholder="Nombre"
                        className="flex-1 bg-black/50 border border-white/10 rounded-xl p-3 text-xs text-white"
                        value={tempFile.name}
                      ...
                      <input
                        ...
                        className="flex-1 bg-black/50 border border-white/10 rounded-xl p-3 text-xs text-white"
                        value={tempFile.url}
                        onChange={(e) => setTempFile({ ...tempFile, url: e.target.value })}
                      />
                      <button
                    ...
                    <div className="space-y-1">
                      {files.map((f, i) => (
                        <div
                          key={i}
                          className="flex justify-between bg-white/5 p-2 rounded text-xs"
                        >
                  ...
                  <textarea
                    className="w-full bg-black border border-white/10 rounded-xl p-4 text-white h-24 outline-none resize-none focus:border-white/30"
                ...
                <div className="lg:col-span-7 flex flex-col h-full lg:pl-10 lg:border-l border-white/5">
                  <h3 className="text-xs font-bold text-emerald-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <ShoppingCart size={14} /> Carrito de Compra
                  </h3>
                  ...
                  <div className="flex bg-[#141414] p-1.5 rounded-2xl mb-6 border border-white/5 w-fit">
                    <button
                      ...
                      className={`px-6 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${itemTab === 'product' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
                    >
                      <Package size={14} /> PRODUCTO
                    </button>
                    <button
                      onClick={() => setItemTab('custom')}
                      className={`px-6 py-2.5 rounded-xl text-xs font-bold transition-all flex items-center gap-2 ${itemTab === 'custom' ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}
                    >
                      <Wrench size={14} /> PERSONALIZADO
                    </button>
                  </div>
                  ...
                  >
                      ...
                      <>
                        <div className="flex flex-wrap gap-2 items-center w-full">
                          <input
                            ...
                            onChange={(e) =>
                              setItemInput({ ...itemInput, customName: e.target.value })
                            }
                          />
                          <div className="relative w-32 min-w-[7rem] border border-white/10 rounded-xl bg-zinc-900/80">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setItemInput({ ...itemInput, customPrice: e.target.value })
                              }
                            />
                          </div>
                          <div className="w-20 min-w-[5rem] border border-white/10 rounded-xl bg-zinc-900/80">
                            <input
                              ...
                              value={itemInput.qty}
                              onChange={(e) =>
                                setItemInput({ ...itemInput, qty: Number(e.target.value) })
                              }
                            />
                          </div>
                          <div
                            className="w-24 min-w-[6rem] border border-white/10 rounded-xl bg-zinc-900/80 flex items-center justify-center gap-1 px-1"
                            title="Tiempo de impresión (minutos)"
                          >
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setItemInput({
                                  ...itemInput,
                                  printTimeMinutes: Number(e.target.value),
                                })
                              }
                            />
                          </div>
                        ...
                        <div className="rounded-xl border border-purple-500/25 bg-zinc-950/90 p-3">
                            ...
                            {orderProductTypeOptions.map((t, optIdx) => {
                              ...
                              return (
                                <button
                                  ...
                                  type="button"
                                  onClick={() =>
                                    setItemInput({ ...itemInput, productType: t.value })
                                  }
                          ...
                          <p className="text-[10px] text-zinc-400 mt-2 leading-relaxed">
                            ...
                            : cada fila (código + nombre + video) aparece acá como tipo. Guardá en
                            Inicio y reabrí este pedido para refrescar.
                          </p>
                        </div>
                      </>
                    )}
                    ...
                    {itemTab === 'product' && (
                      <>
                        ...
                        <div
                          className="w-24 border-l border-white/10 flex items-center justify-center gap-1 px-1"
                          title="Tiempo de impresión (minutos)"
                        >
                  ...
                  <div className="flex-1 bg-[#141414] rounded-2xl border border-white/5 overflow-hidden flex flex-col mb-6">
                    <div className="overflow-y-auto flex-1 custom-scrollbar p-3 space-y-2">
                      {cart.map((i, idx) => (
                        <div
                          key={idx}
                          className="flex justify-between items-center p-3 bg-black/40 rounded-xl border border-white/5 hover:border-white/20 transition-all group"
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`p-2.5 rounded-xl ${i.isCustom ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400'}`}
                            >
                              {i.isCustom ? <Wrench size={16} /> : <Package size={16} />}
                            </div>
                            <div>
                              <p className="font-bold text-sm text-white">{i.productName}</p>
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <button
                              ...
                              className="text-gray-600 hover:text-red-400 bg-white/5 p-2 rounded-lg hover:bg-white/10 transition-colors"
                            >
                              <Trash2 size={16} />
                            </button>
                          </div>
                      ...
                      {cart.length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center text-gray-600 opacity-30 gap-2">
              ...
              <div className="p-6 border-t border-white/10 bg-[#141414] flex justify-end gap-3 rounded-b-3xl">
                <button
                  ...
                  className="px-6 py-3 rounded-xl text-xs font-bold text-gray-500 hover:text-white uppercase tracking-widest transition-colors"
                >
                  Cancelar
                </button>
                <button
                  onClick={handleSaveOrder}
                  className="bg-white text-black px-10 py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-gray-200 transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:scale-105 active:scale-95 flex items-center gap-2"
                >
            ...
            <div className="w-full max-w-lg bg-gradient-to-br from-purple-900/80 to-black border border-purple-500/30 rounded-3xl p-8 shadow-2xl shadow-purple-500/20">
              <div className="text-center mb-6">
                ...
                <h2 className="text-2xl font-black text-white mb-2">
                  ¡Código de Seguimiento Creado!
                </h2>
                <p className="text-purple-300 text-sm">
                  Este código sirve para que el cliente rastree su pedido y vea el video
                  personalizado
                </p>
              </div>

              <div className="bg-black/50 rounded-2xl p-6 mb-6 border border-purple-500/20">
                <label className="text-[10px] font-bold text-purple-400 uppercase tracking-widest block mb-3 text-center">
                ...
                <p className="text-xs text-gray-500 text-center mt-3">
                  El cliente debe buscar este código en /track
                </p>
              </div>
              ...
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 mb-6">
                ...
                <p className="text-gray-400 text-xs">
                  Para que el cliente pueda ver el video, necesitas:
                </p>
                <ol className="text-xs text-gray-500 mt-2 space-y-1 list-decimal list-inside">
                  ...
                  <li>
                    En &quot;Videos por código&quot;, usar el mismo texto que el tipo de producto o
                    el nombre (ej: <span className="text-purple-300">vasoriver</span> o{' '}
                    <span className="text-purple-300">vaso-boca</span>); varios clientes con el
                    mismo tipo verán el mismo video
                  </li>
...
function OrderCard({ order, activeTab, onEdit, onDeliver, onStatusChange, onPay, onPrint }: any) {
  ...
  return (
    <div
      className={`rounded-3xl p-6 relative group transition-all duration-500 border ${cardStyle}`}
    >
      <div
        className={`absolute left-0 top-0 bottom-0 w-1.5 ${isPrinting ? 'bg-orange-500 shadow-[0_0_15px_#f97316]' : order.status === 'completed' ? 'bg-emerald-500 shadow-[0_0_15px_#10b981]' : order.status === 'delivered' ? 'bg-gray-500' : 'bg-yellow-500'}`}
      />
      ...
      <div className="flex justify-between items-start mb-4 pl-3">
        <div>
          ...
          {order.items?.some((it: any) => it.isCustom) && order.trackingCode && (
            <p
              className="mt-1 text-[10px] font-mono text-purple-300/90 truncate"
          ...
          {order.dueDate && (
            <div className="flex items-center gap-1.5 mt-2 text-[10px] text-gray-500 font-bold uppercase tracking-wide">
              <Calendar
                size={12}
                className={new Date(order.dueDate) < new Date() ? 'text-red-500' : 'text-gray-500'}
              />
          ...
          {order.chatLink && (
            <a
              href={order.chatLink}
              target="_blank"
              className="mt-2 flex items-center gap-1 text-[10px] text-blue-400 hover:text-blue-300 font-bold uppercase tracking-wider hover:underline w-fit"
            >
        ...
        <div className="text-right">
          ...
          {balance > 0 ? (
            <button
              onClick={onPay}
              className="text-[10px] text-red-400 font-bold uppercase bg-red-500/10 px-2 py-0.5 rounded hover:bg-red-500/20 transition-colors flex items-center gap-1 ml-auto"
            >
      ...
      <div className="bg-black/30 rounded-xl p-3 mb-4 space-y-2 border border-white/5 pl-3 min-h-[60px]">
        {order.items.slice(0, 3).map((item: any, i: number) => (
          <div key={i} className="flex justify-between text-xs text-gray-300">
            ...
            <span className="font-bold text-white bg-white/10 px-1.5 rounded">
              {item.quantity}x
            </span>
          </div>
        ))}
      ...
      {(order.notes || (order.files && order.files.length > 0)) && (
        <div className="mb-4 pl-3 flex gap-2">
          {order.notes && (
            <div
              className="bg-yellow-500/10 text-yellow-500 p-1.5 rounded-lg border border-yellow-500/20"
              title={order.notes}
            >
              <FileText size={14} />
            </div>
      ...
      <div className="flex flex-wrap gap-2 pl-3">
        <button
          onClick={onEdit}
          className="flex-1 min-w-[80px] py-3 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-bold text-gray-400 hover:text-white transition-colors border border-white/5 uppercase tracking-wider"
        >
          Editar
        </button>

        {activeTab === 'production' && order.status === 'pending' && (
          <button
            onClick={() => onStatusChange(order._id, 'in_progress')}
            className="flex-[2] min-w-[130px] py-3 rounded-xl bg-orange-600 hover:bg-orange-500 text-white text-xs font-bold transition-all flex items-center justify-center gap-2 uppercase tracking-wider"
          >
            <Flame size={16} /> Empezar
          </button>
        )}

        {activeTab === 'production' && order.status === 'in_progress' && (
          <button
            onClick={() => onStatusChange(order._id, 'completed')}
            className="flex-[2] min-w-[130px] py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all flex items-center justify-center gap-2 uppercase tracking-wider"
          >
            <PackageCheck size={16} /> Marcar listo
          </button>
        )}
        ...
        {activeTab === 'production' && (
          <button
            onClick={onPrint}
            title="Marcar ítem como impreso"
            className="px-3 py-3 rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white transition-colors border border-white/5"
          >
            <Factory size={16} />
          </button>
        )}
        ...
        {activeTab === 'ready' && !order.isSaleRegistered && (
          <button
            onClick={onDeliver}
            className="flex-[2] min-w-[130px] py-3 rounded-xl bg-green-600 hover:bg-green-500 text-white text-xs font-bold transition-all shadow-lg shadow-green-900/20 flex items-center justify-center gap-2 uppercase tracking-wider"
          >
        ...
        {activeTab === 'history' && (
          <span
            className={`flex-[2] min-w-[130px] py-3 rounded-xl text-xs font-bold uppercase tracking-wider text-center border ${statusInfo.color} border-opacity-30`}
          >
            {statusInfo.label}
          </span>
        )}
