  ...
  const loadData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setSectionData(data.filamentosSection || {
          enabled: true,
          title: "Filamentos y Materiales",
          subtitle: "PLA, PETG, ABS y más",
          badge: "🧵 FILAMENTOS",
          heroImage: "",
          categories: []
        });
      ...
      console.error("Error loading filamentos:", e);
    } finally {
      setLoading(false);
    }
  };
      ...
      {allProducts.length === 0 && categories.length === 0 ? (
        <div className="min-h-screen bg-black flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-500">Todos los colores y materiales</p>
          </div>
        </div>
      ) : (
        <CollectionPage
          title={sectionData?.title || "Filamentos y Materiales"}
          subtitle={sectionData?.subtitle || "PLA, PETG, ABS y más"}
          badge={sectionData?.badge || "🧵 FILAMENTOS"}
